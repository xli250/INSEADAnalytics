...
* 1. There are some high correlations between the responses. For example, pairs of questions with correlation higher than 0.5 include question 3 and 9, question 6 and 7, question 9 and 26, question 13 and 14, question 14 and 15, question 16 and 17, question 16 and 20, question 16 and 28, question 20 and 27, question 20 and 28. The patterns make sense as the pairs essentially examine similar attributes of boat owners. Take 3 and 9 as an example, the brand of the boat saying a lot about who the boat owner is is essentially similar to perceiving the boat as a status symbol. Similar conclusions can be made for other pairs.
* 2. The correlations indicate how likely it is to see similar responses to each pair of questions asked. For example, it is not likely to have a boat owner who cares the most about price to be also willing to pay a premium for a reputable brand because the correlation between 2 and 4 is -0.21.  
...
**Answers**
  ...
  * 1. The variance explained table ranks the components in order of their explanatory power. Factors with high eigenvalue capture more of the information, i.e. variance, of the original raw data. The higher the eigenvalue, the more information the factor has than the average original raw attribute. % of explained variance means how much each components accounts for the total variance in the original raw data. The higher the % of variance explained, the more a component accounts for total variance. This also links with the eigenvalues of the components. The plot visualize the table with eigenvalues of the components as the blue line and the benchmarking line with constant value of 1 as the orange line. Any blue dot that is above the orange line has more information that the average original raw attribute. 
* 2. The plot is in order of eigenvalue of the components with eigenvalues decreasing from the highest to the lowest. As such, the shape or trend of the blue line is always downward sloping. The plotted line cannot be increasing due to the ranking of the components according to their eigenvalues.
* 3. Our goal here is to have a small or smaller number of derived factors. As such we want to use only a few of the components without losing much information. Ideally, we would prefer to see few factors that have high eigenvalues, i.e. the explanatory power of the top factors concentrated. 
...
* 1. The first couple of factors have more explanatory power in total variance of the raw data. They are the most important to boat owners' attitudes on boating. We have labeled the first couple of factors (see answer to question 3). They make business sense as they imply different attitudes that boat owners have towards boating. These factors help us segment different types of potential customers and tell us what are the most important criteria in their purchasing decisions.
* 2. We have experimented with different numbers of factors and decided to choose 12 factors. Statistically, we want to achieve a high level of variance explained of the factors, i.e. 70%+. The first 12 factors in total explain 71.2% of the variance. We also tried iterating with fewer factors but think 12 would be optimal for both interpretation and business aspects. For interpretation, we want to have a relatively comprehensive coverage of  factors that explain different customer attitudes / behaviors. Having only 5 factors, for example (what we did earlier in class), miss out factors such as DIY, brand, functionality that are not covered or have very little overlap with the first 5 factors. We feel that 12 factors empower us adequately to interpret boat owners' rationale. From a business perspective, 12 factors also make more sense when we conduct customer clustering analysis as it gives us a good way to segment the customers compared to more ambigious segmentation results we get from only having 5 factors. Additionally, if we go with too many factors, say 20, then it defeat the purpose of factoring analysis in a way as ultimately we want to have fewer factors that are easily to measure to facilitate our business decisions. 
* 3. We have labeled the first couple of factors as follows: Component 1: passion for boating, Component 2: technology savvy, Component 3: escapism, Component 4: status seeking, Component 5: budget, Component 6: DIY, Component 7: brand, Component 8: expert opinion, Component 9: functionality, Component 10: shopping for value, Component 11: low engagement, Component 12: socializing. 
* 4. Data science is about experimenting with data and iteration. A good answer is not likely to be obtain at the first trial and more insights will be gained during the iteration process itself. The more iterations we do, the more we seem to understand the data and boat owners' attitude. On the other hand, there is no perfect answer as we are always constrained by the conflict between simplicity of results (fewer factors) and the need to have decent explanatory power. We need to use judgement to come up with the choice of factors to generate logical, reasonable and easy-to-implement methods that can help with business decisions.  
...
* 1. Obs.01: someone who is wealthy, has a big budget and free time, seeks status through boating and does not have too much intention to DIY. In contrast, Obs.10: someone who is very passionate about boating, values the escapism of boating but has a budget so is not likely to purchase expensive and fancy boats.
* 2. We would select the initial variables that have the highest weights for the 12 factors. Component 1: Q1.27, Component 2: Q1.13, Component 3: Q1.19, Component 4: Q1.7, Component 5: Q1.2, Component 6: Q1.11, Component 7: Q1.12, Component 8: Q1.10, Component 9: Q1.25, Component 10: Q1.1, Component 11: Q1.29, Component 12: Q1.8.
...
* 1. The numbers represent how much each segment varies from the population average with respect to each profiling variables. Applying the P&G rule, we identified that the extreme values, especially the ones with greater or lesser value such as Q 10 and 16, are the most informative for the purpose of defining segment characteristic.
* 2. Segment 1: Beginner boaters – This segment spend the least amount of time on the boat per year, at 24.84 days compare to 46.25 days in average across 4 segments, and have little to no regard for brand equity
* Segment 2: Casual boaters – This segment also boat less than the population average number of days per year at 31.06 days but care deeply about brand equity, more so than the other segments
* Segment 3: Passionate boaters – This segment go boating as their preferred activities for their free-time. Based on our analysis for Q 1.2, this segment care less about brand than value. 
* Segment 4: Professional boaters – This segment use their boat for majority of the year and have little preference or loyalty towards brand equity.
...
* 1. We believe there are 4 segments in our market, why? 
* 1.a. Statistical arguments: The elbow in the chart plotting distances on the dendrogram falls between 4 and 5 variables. At 3 variables the distance is 238.45; while at 4 variables is 196.75 and 5 variables is 189.44. We choose to proceed with 4 variables because the distance between 4 and 5 is not as significant
* 1.b. Interpretation arguments: We reviewed the market using 5 segments; however, the information we received are coherent
* 1.c. Business arguments: It is more visible, both from cost and technological perspective, for the boating company to introduce 4 new boats to 4 distinct company. The more differentiated or unique the products offering are, the better it is for the company.
*
*
*
*
